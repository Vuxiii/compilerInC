...
// Maybe replace with their corresponding REG_XXX from `codegen.h`
...
#define REG64(runtime, reg) ((runtime)->registers)[(reg)]

#define PUSHQ(runtime, value) (runtime)->stack[(RSP(runtime)--)] = value;
#define POPQ(runtime, register) (register) = *((runtime)->stack + (++RSP(runtime)))
#define ENTER(runtime, offset) PUSHQ((runtime), RBP(runtime));\
                               SUB(offset, RSP(runtime));\
                               MOVE(RSP(runtime), RBP(runtime))
#define LEAVE(runtime) MOVE(RBP(runtime), RSP(runtime))
// #define RET(runtime) 
// #define CALL(runtime, function)
#define MOVQ(from, to) (to) = (from)
...
#define OFFSET(amount, reg, type) *(type *)((reg) + amount)
